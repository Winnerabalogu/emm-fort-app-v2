// components/creator/settings/CreatorPersonalSettings.tsx
"use client";
...
import { User, Mail, Phone, AtSign, CheckCircle, AlertCircle, Info, Instagram, MessageCircle, Users } from 'lucide-react';
...
export default function CreatorPersonalSettings() {
  const [isLoading, setIsLoading] = useState(true);
  const [isSaving, setIsSaving] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [successMessage, setSuccessMessage] = useState<string | null>(null);

  const { register, handleSubmit, reset, formState: { errors, isDirty } } = useForm<CreatorProfileData>();
  ...
  const onSubmit: SubmitHandler<CreatorProfileData> = async (data) => {
    setIsSaving(true);
    setError(null);
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-orange-50 to-amber-50 rounded-xl p-6 sm:p-8 border border-orange-100">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            <h2 className="text-xl sm:text-2xl font-bold text-gray-900">Creator Profile</h2>
            <p className="text-gray-600 mt-1">Manage your creator profile and social media links for better audience connection.</p>
          </div>
        </div>
      </div>
      ...
      {/* Social Media & Creator Info */}
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 sm:p-8">
        <h3 className="text-lg font-semibold text-gray-900 mb-6">Social Media & Creator Details</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* Instagram Handle */}
          <div className="space-y-2">
            <label className="flex items-center gap-2 text-sm font-medium text-gray-700">
            ...
            <div className="relative">
              <Input
                ...
                style={{ paddingLeft: '2.5rem' }}
              />
              <Instagram className="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-gray-400 pointer-events-none" />
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              <Input
                ...
                style={{ paddingLeft: '2.5rem' }}
              />
              <MessageCircle className="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-gray-400 pointer-events-none" />
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <select
              ...
              className="block w-full px-3 py-2.5 border border-gray-300 rounded-lg bg-white text-gray-900 focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20"
            >
